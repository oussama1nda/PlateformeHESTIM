...
<div class="space-y-6">
    <div class="bg-white rounded-lg shadow p-6">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Créer un Nouveau Créneau</h2>
            <p class="text-sm text-gray-600 mt-1">Ajoutez un nouveau créneau à l'emploi du temps</p>
        </div>

        <form action="{{ route('admin.emploi-du-temps.store') }}" method="POST" id="emploiForm" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type_seance" id="type_seance" required
                        ...
                        <option value="td" {{ old('type_seance') == 'td' ? 'selected' : '' }}>TD (Travaux Dirigés)</option>
                        <option value="tp" {{ old('type_seance') == 'tp' ? 'selected' : '' }}>TP (Travaux Pratiques)</option>
                    </select>
                    @error('type_seance')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.getElementById('emploiForm');
    ...
    checkConflictBtn.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.has_conflict) {
                ...
                alert('Aucun conflit détecté. Vous pouvez créer ce créneau.');
            }
        })
        .catch(error => {
